...
class SynchronizeYahooAuction():
    ...
    def _process_batch(self, items, active_status, end_status, ebay_end_status, ebay_active_status):
        """
        バッチ単位でアイテムを処理
        """
        updated_items = []
        ...
        for item in items:
            try:
                ...
                if detail.get('end_flag'):
                    ...
                    # ebay_items = getattr(item, 'prefetched_ebay', [])
                    # ebay_item = next((e for e in ebay_items), None)
                    # ebay_item.status = ebay_active_status
                    # ebay_item.save()
    ...
    def synchronize(self, yahoo_auction_id: int = None):
        ...
        try:
            ...
            with transaction.atomic():
                ...
                ebay_active_status = StatusModel.objects.get(id=1)
                ...
                while True:
                    ...
                    result = self._process_batch(batch, active_status, end_status, ebay_end_status, ebay_active_status)
